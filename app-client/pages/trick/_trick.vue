  ...
  <item-content-layout>
    <template v-slot:content>
      <div v-if="submissions">
        <v-card v-for="s in submissions" :key="`${trick.id}-${s.id}`">
          <v-card-title>{{s.id}} - {{s.trickId}}</v-card-title>
    ...
    <template v-slot:item>
      <div class="text-6">
        ...
        <v-chip small class="mb-1 ml-2" :to="`/difficulty/${difficulty.id}`">{{difficulty.name}} Difficulty</v-chip>
      </div>
