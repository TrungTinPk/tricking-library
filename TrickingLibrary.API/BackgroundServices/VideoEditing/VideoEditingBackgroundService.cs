...
namespace TrickingLibrary.API.BackgroundServices.VideoEditing
{
    public class VideoEditingBackgroundService : BackgroundService
    {
        ...
        private ChannelReader<EditVideoMessage> _channelReader;
            ...
            while (await _channelReader.WaitToReadAsync(stoppingToken))
            {
                ...
                try
                {
                    var startInfo = new ProcessStartInfo
                    {
                        ...
                        Arguments = $"-i {inputPath} -an -vf scale=540x380 -y {outputConvertedPath} -ss 00:00:00 -vframes 1 -vf scale=540x380 {outputThumbnailPath}",
                        CreateNoWindow = true,
                    ...
                    if (!_videoManager.TemporaryVideoExits(outputConvertedName))
                    {
                        throw new Exception("FFMPEG failed to generate converted video");
                    }
                    
                    if (!_videoManager.TemporaryVideoExits(outputThumbnailName))
                    {
